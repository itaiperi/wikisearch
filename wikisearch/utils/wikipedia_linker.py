...
def get_nonexistent_wikipedia_pages(wiki_language):
    ...
    with open(os.path.join("wikisearch", "data_for_visibility", "nonexistent_wikipedia_pages"), 'w', encoding='utf-8') \
        ...
        print(f"Getting the nonexistent pages took: {int(time.time() - start)} seconds")
...
def get_redirect_pages():
    ...
    print(f"Getting the redirect pages took: {int(time.time() - start)} seconds")
...
def build_redirect_pages_lookup_table():
    ...
    print(f"Building the redirect lookup table took: {int(time.time() - start)} seconds")
